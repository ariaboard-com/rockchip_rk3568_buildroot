...
if [ ! -e "/data/cfg/rockchip_test/auto_recovery.sh" ]; then
	cp /rockchip_test/auto_reboot/auto_recovery.sh /data/cfg/rockchip_test
fi
...
then
    ...
    echo "off" > $CNT
    echo "do cleaning ..."
    rm -rf /data/cfg/rockchip_test/auto_recovery.sh
	rm -f $CNT
    exit 0
fi
